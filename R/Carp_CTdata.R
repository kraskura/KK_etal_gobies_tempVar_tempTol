...
for (i in 1:nrow(data)){
  if(is.na(data$tank[i])){
   data$tank[i]<-as.character(data$Date.1)[i] 
  }
}

data$temp<-as.numeric(as.character(data$temp_treatment)) # NA are variable treatments 
data$tank<-as.factor(as.character(data$tank)) # NA are variable field treatments 
...
## 2. [main 3a] LAB & STATIC ONLY  (continuous temp predictor) --------
...
mod1min<-lmer(temp_tolerance ~  temp + (1|tank), REML = FALSE, data = data.static.min) 
mod1min.b<-lmer(temp_tolerance ~  temp + TL_mm + (1|tank), REML = FALSE, data = data.static.min) 
# plot(mod1min)
qqmath(mod1min.b)
summary(mod1min.b)
...
## 3. [main 3b] ALL TESTS: ANOVA between all tests -----------
...
mod.CTmin<-lmer(temp_tolerance ~ treatment + (1|tank), data = data[data$Test== "CTmin", ])
mod.CTmin.b<-lmer(temp_tolerance ~ TL_mm + treatment + (1|tank), data = data[data$Test== "CTmin", ])
BICdelta(BIC(mod.CTmin, mod.CTmin.b))

car::Anova(mod.CTmin.b, type = "II")
# plot(mod.CTmin.b)
  
mod.CTmax<-lmer(temp_tolerance ~ treatment + (1|tank), data = data[data$Test== "CTmax", ])
mod.CTmax.b<-lmer(temp_tolerance ~ TL_mm + treatment + (1|tank), data = data[data$Test== "CTmax", ])
...
## 4. [suppl 3b] FIELD vs LAB: variable treatments -----------
...
mod.var.CTmin.b<-lmer(temp_tolerance ~ Field_Lab + TL_mm + (1|treatment:tank), data = data.VARmin, REML = FALSE)
mod.var.CTmin.b2<-lmer(temp_tolerance ~ Field_Lab + TL_mm + (1|treatment), data = data.VARmin, REML = FALSE)

BICdelta(BIC(mod.var.CTmin, mod.var.CTmin.b2, mod.var.CTmin.b))
...
mod.var.CTmax.b<-lmer(temp_tolerance ~ Field_Lab + TL_mm + (1|treatment:tank), data = data.VARmax, REML = FALSE)
mod.var.CTmax.b2<-lmer(temp_tolerance ~ Field_Lab + TL_mm + (1|treatment), data = data.VARmax, REML = FALSE)

BICdelta(BIC(mod.var.CTmax, mod.var.CTmax.b, mod.var.CTmax.b2))
...
## 5. [main 4ab]  ALL TESTS: continuous , static and variable.  max, mean, min, delta, start temp, start daytime -----
...
# model.CTmin.7.b<-lmer(temp_tolerance ~ max.Env.Temp + delta.T + TL_mm + (1|treatment), data = data[data$Test== "CTmin", ], REML = FALSE) 
# model.CTmin.7.d<-lmer(temp_tolerance ~ max.Env.Temp + delta.T + TL_mm + Stat_Var + (1|treatment), data = data[data$Test== "CTmin", ], REML = FALSE) 
...
# *** 
model.CTmin.0<-lmer(temp_tolerance ~ 1  +(1|treatment:tank), data = data[data$Test== "CTmin", ], REML = FALSE)
model.CTmin.1<-lmer(temp_tolerance ~ max.Env.Temp +(1|treatment:tank), data = data[data$Test== "CTmin", ], REML = FALSE) 
model.CTmin.b<-lmer(temp_tolerance ~ max.Env.Temp + TL_mm +(1|treatment:tank), data = data[data$Test== "CTmin", ], REML = FALSE) 
model.CTmin.2<-lmer(temp_tolerance ~ min.Env.Temp +(1|treatment:tank), data = data[data$Test== "CTmin", ], REML = FALSE) 
model.CTmin.2.b<-lmer(temp_tolerance ~ min.Env.Temp + TL_mm +(1|treatment:tank), data = data[data$Test== "CTmin", ], REML = FALSE) 
model.CTmin.3<-lmer(temp_tolerance ~ mean.Env.Temp +(1|treatment:tank), data = data[data$Test== "CTmin", ], REML = FALSE) 
model.CTmin.3.b<-lmer(temp_tolerance ~ mean.Env.Temp +TL_mm + (1|treatment:tank), data = data[data$Test== "CTmin", ], REML = FALSE) 
model.CTmin.4<-lmer(temp_tolerance ~ Temp_test_start +(1|treatment:tank), data = data[data$Test== "CTmin", ], REML = FALSE) 
model.CTmin.4.b<-lmer(temp_tolerance ~ Temp_test_start + TL_mm +(1|treatment:tank), data = data[data$Test== "CTmin", ], REML = FALSE) 
model.CTmin.5<-lmer(temp_tolerance ~ delta.T +(1|treatment:tank), data = data[data$Test== "CTmin", ], REML = FALSE) 
model.CTmin.5.b<-lmer(temp_tolerance ~ delta.T + TL_mm +(1|treatment:tank), data = data[data$Test== "CTmin", ], REML = FALSE) 
model.CTmin.6<-lmer(temp_tolerance ~ TimeDay2 +(1|treatment:tank), data = data[data$Test== "CTmin", ], REML = FALSE) 
model.CTmin.6.b<-lmer(temp_tolerance ~ TimeDay2 + TL_mm +(1|treatment:tank), data = data[data$Test== "CTmin", ], REML = FALSE) 
model.CTmin.7<-lmer(temp_tolerance ~ max.Env.Temp + delta.T + (1|treatment:tank), data = data[data$Test== "CTmin", ], REML = FALSE) 
model.CTmin.7.c<-lmer(temp_tolerance ~ max.Env.Temp + delta.T + Stat_Var + (1|treatment:tank), data = data[data$Test== "CTmin", ], REML = FALSE) 
  model.CTmin.7.b<-lmer(temp_tolerance ~ max.Env.Temp + delta.T + TL_mm + (1|treatment:tank), data = data[data$Test== "CTmin", ], REML = FALSE) # BEST final ms revisions oct 23
model.CTmin.7.d<-lmer(temp_tolerance ~ max.Env.Temp + delta.T + TL_mm + Stat_Var + (1|treatment:tank), data = data[data$Test== "CTmin", ], REML = FALSE) 
model.CTmin.8<-lmer(temp_tolerance ~ max.Env.Temp + Temp_test_start + (1|treatment:tank), data = data[data$Test== "CTmin", ], REML = FALSE) 
model.CTmin.9<-lmer(temp_tolerance ~ delta.T + Temp_test_start + (1|treatment:tank), data = data[data$Test== "CTmin", ], REML = FALSE) 
model.CTmin.10<-lmer(temp_tolerance ~ max.Env.Temp + delta.T + TimeDay2 + (1|treatment:tank), data = data[data$Test== "CTmin", ], REML = FALSE) 
model.CTmin.11<-lmer(temp_tolerance ~ max.Env.Temp + Temp_test_start + delta.T + (1|treatment:tank), data = data[data$Test== "CTmin", ], REML = FALSE)
...
BICdelta(BIC(model.CTmin.0, model.CTmin.1, model.CTmin.2, model.CTmin.3, model.CTmin.4, model.CTmin.5, model.CTmin.6,
             model.CTmin.b, model.CTmin.2.b, model.CTmin.3.b, model.CTmin.4.b, model.CTmin.5.b, model.CTmin.6.b,
             model.CTmin.7,model.CTmin.7.d, model.CTmin.7.b, model.CTmin.7.c,
             model.CTmin.8, model.CTmin.9, model.CTmin.10, model.CTmin.11))


### [suppl TABL 2] ----------
car::Anova(model.CTmin.7.b, type = "II") #CTmin
...
# model.CTmax.3.b<-lmer(temp_tolerance ~ mean.Env.Temp + TL_mm + (1|treatment), data = data[data$Test== "CTmax", ], REML = FALSE)
...
# treatment:tank
model.CTmax.0<-lmer(temp_tolerance ~ 1  + (1|treatment:tank), data = data[data$Test== "CTmax", ], REML = FALSE)
model.CTmax.1<-lmer(temp_tolerance ~ max.Env.Temp + (1|treatment:tank), data = data[data$Test== "CTmax", ], REML = FALSE) 
model.CTmax.1.b<-lmer(temp_tolerance ~ max.Env.Temp + TL_mm + (1|treatment:tank), data = data[data$Test== "CTmax", ], REML = FALSE) 
model.CTmax.2<-lmer(temp_tolerance ~ min.Env.Temp + (1|treatment:tank), data = data[data$Test== "CTmax", ], REML = FALSE) 
model.CTmax.2.b<-lmer(temp_tolerance ~ min.Env.Temp + TL_mm + (1|treatment:tank), data = data[data$Test== "CTmax", ], REML = FALSE) 
model.CTmax.3<-lmer(temp_tolerance ~ mean.Env.Temp + (1|treatment:tank), data = data[data$Test== "CTmax", ], REML = FALSE) 
  model.CTmax.3.b<-lmer(temp_tolerance ~ mean.Env.Temp + TL_mm + (1|treatment:tank), data = data[data$Test== "CTmax", ], REML = FALSE) # best model revisions oct 23
model.CTmax.3.c<-lmer(temp_tolerance ~ mean.Env.Temp + TL_mm + delta.T + (1|treatment:tank), data = data[data$Test== "CTmax", ], REML = FALSE) 
model.CTmax.4<-lmer(temp_tolerance ~ Temp_test_start + (1|treatment:tank), data = data[data$Test== "CTmax", ], REML = FALSE) 
model.CTmax.4.b<-lmer(temp_tolerance ~ Temp_test_start + TL_mm + (1|treatment:tank), data = data[data$Test== "CTmax", ], REML = FALSE) 
model.CTmax.5<-lmer(temp_tolerance ~ delta.T + (1|treatment:tank), data = data[data$Test== "CTmax", ], REML = FALSE) 
model.CTmax.5.b<-lmer(temp_tolerance ~ delta.T + TL_mm + (1|treatment:tank), data = data[data$Test== "CTmax", ], REML = FALSE) 
model.CTmax.6<-lmer(temp_tolerance ~ TimeDay2 + (1|treatment:tank), data = data[data$Test== "CTmax", ], REML = FALSE) 
model.CTmax.6.b<-lmer(temp_tolerance ~ TimeDay2 + TL_mm + (1|treatment:tank), data = data[data$Test== "CTmax", ], REML = FALSE) 
model.CTmax.7<-lmer(temp_tolerance ~ max.Env.Temp + delta.T + (1|treatment:tank), data = data[data$Test== "CTmax", ], REML = FALSE) 
model.CTmax.7.b<-lmer(temp_tolerance ~ max.Env.Temp + delta.T + TL_mm + (1|treatment:tank), data = data[data$Test== "CTmax", ], REML = FALSE) 
model.CTmax.8<-lmer(temp_tolerance ~ max.Env.Temp + Temp_test_start + (1|treatment:tank), data = data[data$Test== "CTmax", ], REML = FALSE) 
model.CTmax.9<-lmer(temp_tolerance ~ delta.T + Temp_test_start + (1|treatment:tank), data = data[data$Test== "CTmax", ], REML = FALSE) 
model.CTmax.10<-lmer(temp_tolerance ~ max.Env.Temp + delta.T + TimeDay2 + (1|treatment:tank), data = data[data$Test== "CTmax", ], REML = FALSE) 
model.CTmax.11<-lmer(temp_tolerance ~ max.Env.Temp + Temp_test_start + delta.T + (1|treatment:tank), data = data[data$Test== "CTmax", ], REML = FALSE)
...
39.70870 - 7.721429 # jul 5- tests 1,2
35.97778 - 8.265217# jul 9- tests 3,4
38.93333 - 5.763636 # jul 11- test 6, 5
38.78500 - 6.079167 # jul 14., tests 7, 8
...
data %>%
  ...
  group_by(TestID, TestID2, Test) %>% 
  summarise(mean = mean(Temp_test_start))


# FIGURES:  ----------
## 3. [main 4] Figures main, all CTs with correlations of env. temp-----
## [not in main ms] start temp *************************************
...
CTplotFIELD1<-ggplot(data=data, aes(y=temp_tolerance, x=max.Env.Temp,  shape = Stat_Var, alpha = Stat_Var, size = Stat_Var, fill = temp_treatment, color = temp_treatment))+
  ...
  annotate(geom = "text", y = -5, x = 11.5, hjust = 0, color = "black", size = 3.5,
         label = bquote( CT[min] == ~ .(CTmin.int) ~ "+" ~ .(CTmin.slopeMax) * T[max] ~ .(CTmin.slopeDelta) * Delta*T ~ .(CTmin.slopeTL) * "TL"))
...
cowplot:::plot_grid(CTplotFIELD2, CTplotFIELD1,
                    ...
                    label_y = c(0.9, 0.9),
                    rel_widths = c(1,1)) %>% 
ggsave(filename = "Figures/Figure4_EnvCorrelations.png", width = 8, height = 4)
...
#                                  rel_widths = c(1, 0.2),
#                                  align = "h")
# ggsave(plot.size1, filename = "./Figures/Not_in_ms_growth.png", width = 6, height = 4.5)
# 

## 2. [suppl 4b]: Figures CTmax CT between-test variation: --------
...
ggsave( filename = "./Figures/FigureS1_scaling_TLmm.png", width = 8, height = 4)
