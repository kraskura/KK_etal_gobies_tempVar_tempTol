...
# Import functions that help with consitent formatting ----------
source("./R/extractTimeFrame.R")
source("./R/transformTempData.R")
source("./R/dataMutateMeans.R")
...
dataLab.V1.plot <- extractTimeFrame(data = dataLab.V1, year = 2019, minMo = 10, maxMo = 10, minDay = 19, maxDay = 26)  # regular variable treatment 
...
daily.p.ALL <- ggplot(dataLab, aes(minutes.day, TEMP, group = interaction(treatm), color = treatm))+
  ...
  ylim(5, 35)+
  scale_color_manual(values=c("#00518C", "#00C59E", "#DBA11C", "#BE647D","#3F4756", "#A3ABBD") )+
...
daily.p <- ggplot(dataLab.S.plot, aes(minutes.day/60, TEMP, group = interaction(treatm), color = treatm))+
  ylim(5, 35)+
  scale_color_manual(values=c("#00518C", "#00C59E", "#DBA11C", "#BE647D") )+
...
# HOBO loggers in carp -------------

dataF<-read.csv("./Data/Field_temps_CombinedData2021.csv", header = T)
...
SiteTempsJul<- SiteTempsJul %>% 
  ...
  write.csv(file = "./Data/SiteTempsJul.csv")
SiteTempsSep<- SiteTempsSep %>% 
  filter(SiteID == "TestSite") %>% 
  write.csv(file = "./Data/SiteTempsSep.csv")
...
dataCarp.V1V2<- extractTimeFrame(dataCarp, year = 2019, minMo = 9, maxMo = 10, minDay = 1, maxDay = 31)
...
SMOOTHdaily.p.CarpV1V2 <- ggplot(dataCarp.V1V2[!(dataCarp.V1V2$SiteID == "SITE2"),], aes(minutes.day/60, TEMP, group = interaction(SiteID)))+
  ylim(0, 35)+
  xlim(0, 28)+
  # scale_color_manual(values=c("#A3ABBD", "#3F4756"))+
  annotate("rect", xmin=0, xmax=360/60, ymin=-Inf, ymax=Inf, alpha=0.3, fill="grey90") +
  annotate("rect", xmin=960/60, xmax=1440/60, ymin=-Inf, ymax=Inf, alpha=0.3, fill="grey90") +
  geom_line(aes(minutes.day/60, TEMP, group = interaction(mo, day, SiteID)), size=0.1, alpha=0.4, color = "#926C00")+
  ...
  geom_text(data =dataCarp.V1V2[!(dataCarp.V1V2$SiteID == "SITE2") & !duplicated(dataCarp.V1V2$SiteID), c("SiteID", "meanMEAN") ],
           aes(x = 25.8, y = meanMEAN, group = SiteID, label=paste(meanMEAN)), hjust=0, size = 3, color = "black")+
  geom_text(data = dataCarp.V1V2[!(dataCarp.V1V2$SiteID == "SITE2") & !duplicated(dataCarp.V1V2$SiteID), c("SiteID", "meanMAX") ],
...
ggformat(SMOOTHdaily.p.CarpV1V2, title = "", y_title = expression(Temperature~(degree*C)), x_title = "Time in the day (h)", size_text = 12, print = T)
...
## Figure v) ---------
daily.pFcont<-ggplot(dataCarp, aes(LocalDateTime, TEMP, color = as.numeric(as.factor(SiteID))))+
...
## saving figures -------
ggsave(filename = "./Figures/Figure1_Field_static.png", daily.pFcont, width = 5, height = 3)
